...
trait GeoDocumentStats {
  ...
  def totalToponymCount()(implicit s: Session): Int =
    Annotations.countForGeoDocumentAndStatus(id.get, NOT_VERIFIED, VERIFIED, AMBIGUOUS, MULTIPLE, NO_SUITABLE_MATCH, NOT_IDENTIFYABLE)
    
  /** # of verified annotations in the document **/
  def unverifiedToponymCount()(implicit s: Session): Int =
    Annotations.countForGeoDocumentAndStatus(id.get, NOT_VERIFIED)
  
  /** Ratio of manually processed vs. total toponyms in the document **/
  def completionRatio()(implicit s: Session): Double = {
    val unverified = unverifiedToponymCount
    val total = totalToponymCount
    (total - unverified).toDouble / total
  }
